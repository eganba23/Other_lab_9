...
race_graph1 <- race_graph %>%
  mutate(race_crime = case_when(
    decile_score >= 7 & race == "White" ~ "White and committed crime",
    decile_score < 7 & race == "Black" ~ "White and committed crime"
  ))

race_accurate <- race_graph %>%
  mutate(race_compas_accurate = case_when(
    decile_score >= 7 & race == "White" ~ 1,
    decile_score >= 7 & race == "Black" ~ 0
  ))
...
There's a clear difference in distribution between white and black defendants who were classified as high risk.
